...
class SAC(RLAlgorithm):
    ...
    def _train(self):
        ...
        for self._epoch in range(0, self._n_epochs):
            ...
            f_log.close()
            '''
            if (self._epoch + 1) % 500 == 0:
                print('1, 2, 3 dump the buffer')
                if not os.path.exists('./data'):
                    os.mkdir('./data')
                with open('./data/sac_%d_buffer.pkl' % self._epoch, 'wb') as f:
                    pickle.dump(self._pool.return_all_samples(), f)
            '''
