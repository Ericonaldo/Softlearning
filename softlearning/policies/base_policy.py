...
class BasePolicy(Serializable):
    ...
    def set_always_deterministic(self, deterministic=True):
...
class LatentSpacePolicy(BasePolicy):
    ...
    def actions_np(self, conditions):
        test_conditions = [_.astype(np.float32) for _ in conditions]
        # print("shift: ", self.shift.predict(conditions))
        # print("log_scale_diag: ", self.log_scale_diag.predict(conditions))
        # print("latents_model: ", self.latents_model.predict(conditions))
        # print("raw_act: ", self.raw_act.predict(conditions))
